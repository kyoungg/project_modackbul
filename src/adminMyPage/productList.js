...
addproductBtn.addEventListener(
  "click",
  () => (window.location.href = "/admin-addProductPage.html")
);
...
async function getProductData() {
  const apiUrl = "http://34.64.164.169/api/products";
...
async function insertProductElement() {
  ...
  <div class="container rounded border border-secondary" id="${productId}" name="${Productname}" data-name="${data.name}">
      <div class="row">
        <img class="productImg col" src="http://34.64.164.169/${Productimg}">
...
async function deleteProduct(e) {
  ...
  const apiUrl = `http://34.64.164.169/api/products/${targetName}`; //삭제하고자 하는 상품의 name

  const answer = confirm(`정말 [${targetName}]상품을 삭제하시겠습니까?`);
  if (answer) {
    //API 통신으로 DB에서 상품 삭제
    const res = await fetch(apiUrl, {
      method: "DELETE",
